...
for i in range(len(unique_domains)):
    ...
    for nc in num_classes:
        ...
        samples_per_class.append(len(samples_in_that_class))
        print(f"location: {unique_domains[i]}, class= {nc}, num samples: {len(samples_in_that_class)}")
    total_array.append(samples_per_class)
...
new_df=pd.DataFrame(total_array, index=unique_domains, columns = num_classes)
new_df.to_csv("num samples per class per domain fmow.csv")
